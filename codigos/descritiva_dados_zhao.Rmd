...
```{r}
base_chf_zhao = read_csv("../dados/Data_CHF_Zhao_2020_ATE.csv")
...
duplicates <- base_sub |>
  group_by(CHF,P_kpa,mass_flux_kg_m2_s,x_e_out,d_h_mm,length_mm) |>
  filter(n() > 1) |>
  ungroup()

base_sub = base_sub[!(base_sub %>% 
  select(CHF,P_kpa,mass_flux_kg_m2_s,x_e_out,d_h_mm,length_mm) %>%
  duplicated()),]

write.csv(base_sub,"../dados/dados_modelagem_zhao.csv",row.names = FALSE)
...
jpeg("../dados/dados_pares_sub_zhao.jpeg",width = 17,height = 10.51,units = "cm",res = 300)
plot_pair
dev.off()
